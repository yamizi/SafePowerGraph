import sys

sys.path.append(".")
from utils.io import get_parser
import hashlib
from utils.logging import init_comet
from utils.attacks import get_attack
from runs.cases import run_case
from utils.attacks.search import SearchAttack
import torch_geometric.transforms as T

parser = get_parser()


def run(mutations=["cost", "load_relative"], cases=["case9", "case14", "case30", "case118"],
        nb_train=8000, nb_val=2000, dataset_type="y_OPF", device="cuda", scale=0, cv_ratio=0.2,
        opf=1, project_name="test_perf_v4", use_ray=1, epochs=500, num_samples=200, aggr="mean", cls="sage",
        base_lr=0.1, decay_lr=0.5, hidden_channels=[64, 64], batch_train=256, clamp_boundary=0, use_physical_loss="1_1"
        , weighting="relative", uniqueid="", seed=1, attack=""):
    for mutation in mutations:
        for case in cases:
            experiment = init_comet({"case": case, "mutation": mutation}, project_name)
            experiment.log_parameters({"attack":attack})
            training_case = [[case, nb_train, 0.7, [mutation]]]
            validation_case = [case, nb_val, 0.7, [mutation]]
            path = "./output/test_robustness/" + project_name

            if uniqueid=="":
                hash_path = f"{path}_{training_case}_{validation_case}"
                hash_path = hashlib.md5(hash_path.encode()).hexdigest()
                #hash_path = hash(hash_path)
            else:
                hash_path = uniqueid


            model, train_graphs, train_networks, validation_graphs, valid_networks = run_case(training_cases=training_case, validation_case=validation_case, plot=False,
                     title="Test performance on " + mutation, save_path=path, dataset_type=dataset_type,
                     experiment=experiment, train_batch_size=batch_train, val_batch_size=512, scale=scale,
                     device=device, opf=opf, use_ray=use_ray, uniqueid=hash_path, max_epochs=epochs, cv_ratio=cv_ratio,
                     num_samples=num_samples, aggr=aggr, cls=cls, base_lr=base_lr, decay_lr=decay_lr,
                     hidden_channels=hidden_channels, clamp_boundary=clamp_boundary,
                     use_physical_loss=use_physical_loss, weighting=weighting, seed=seed,
                     return_model_if_exists=1, hetero=True)

            if len(attack)>1:
                i = 0
                params = {"line":train_graphs[0].dataframes.get("line").columns}
                data = validation_graphs[i].data

                data = T.ToUndirected(merge=True)(data)
                model_out = model(data.x_dict, data.edge_index_dict)

                attack_cls = get_attack(model, attack, log=True)

                if isinstance(attack_cls,SearchAttack):
                    args = {"initial_network":validation_graphs[i],"budget" :100, "data":data, "num_actors":1,
                            "popsize":1000}
                else:
                    x = data.x_dict if hasattr(data, "x_dict") else data.x
                    edge_index = data.edge_index_dict if hasattr(data, "edge_index_dict") else data.edge_index
                    y = data.y_dict if hasattr(data, "y_dict") else data.y

                    args = {"budget" :2, "x":x,"y":y,"edge_index":edge_index}  # Degree of (training) node 42 is 2.


                pert_edge_index, perts = attack_cls.attack(**args)
                print("mutation rate",(pert_edge_index==data.edge_index).float().mean())

if __name__ == "__main__":
    args = parser.parse_args()
    mutations = args.mutations.split("+")
    cases = args.cases.split("+")
    hidden_channels = args.hidden_channels.split(":")
    comet_name = args.comet_name if args.comet_name != "" else "test_perf_a1"

    run(mutations, cases, args.nb_train, args.nb_val, device=args.device, scale=args.scale, epochs=args.epochs,
        dataset_type=args.dataset_type, opf=args.opf, project_name=comet_name, use_ray=args.ray,
        num_samples=args.num_samples, cv_ratio=args.cv_ratio, aggr=args.aggr, cls=args.cls, decay_lr=args.decay_lr,
        base_lr=args.base_lr, hidden_channels=hidden_channels, batch_train=args.batch_train,
        clamp_boundary=args.clamp_boundary, use_physical_loss=args.use_physical_loss, weighting=args.weighting,
        uniqueid=args.uniqueid, seed=args.seed, attack=args.attack)
